...
public class TableInvoicePaymentItem extends TableObservable implements ITableMainExt {
	private TableModelPaymentItem modelThuChi;
	...
	public TableInvoicePaymentItem(Date start, Date end, Employee employee, String currency, ActivityType activityType,
		...
		this.types = types;
		this.name = name;
		modelThuChi = new TableModelPaymentItem();
		...
		if (invoiceDetail.getInvoiceCode().startsWith("TC")) {
			...
			if (invoiceDetail.getType().equals(AccountingModelManager.typeBanHang)) {
				...
				Profile profile = AccountModelManager.getInstance().getProfileConfigAdmin();
				 ...
				 DialogPhieuDatMuaHangPos panel =
				 DialogPhieuDatMuaHangPos.getInstance();
				 if (DialogScreenOftenPos.isFlagScreenOften()) {
		...
		for (int i = 0; i < records.size(); i++) {
			...
			for (int j = 0; j < field.length; j++) {
				...
				case 8:
					...
					break;
				case 12:
					System.out.println(field[j]+"   "+ record.get(field[j]).toString());
				...
				case 10:
					try {
						if (!record.get(field[j]).toString().isEmpty()) {
							values[j] = record.get(field[j]).toString();
							values[11] = record.get("mobile").toString();
						}
					} catch (Exception e) {
					}
				...
				case 11:
					try {
						try {
							if (!record.get(field[j]).toString().isEmpty()) {
								values[10] = record.get(field[j]).toString();
				...
				case 26:
					try {
						values[j] = new MyDouble(new MyDouble(values[24].toString()).doubleValue()
						    - new MyDouble(values[25].toString()).doubleValue());
					} catch (Exception e) {
						values[j] = "0";
					}
					break;
			...
			try {
				...
				rQuery
				    .table("InvoiceItem i")
				    .field("i.itemName AS `itemName`", "itemName")
				    .field("i.invoiceCode AS `itemCode`", "itemCode")
				    .field("i.quantity AS `quantity`", "quantity")
				    .field("i.unitPrice AS `unitPrice`", "unitPrice")
				    .field("i.total AS `total`", "total")
				    .cond("i.invoiceId = '" +code + "'");
				ReportTable[] reportTable = AccountingModelManager.getInstance().reportQuery(new SQLSelectQuery[] { rQuery });

				List<Map<String, Object>> records = reportTable[0].getRecords();
				for (int j = 0; j < records.size(); j++) {
					Map<String, Object> record1 = records.get(j);
					String[] strs1 = { "", record1.get("itemCode").toString(),record1.get("itemName").toString(),
							new MyDouble(record1.get("quantity").toString()).toString(), new MyDouble(record1.get("unitPrice").toString()).toString(),
							new MyDouble(record1.get("total").toString()).toString(),"", "", "",
					    "", "", "", "", "", "", "", "", "", "",
					    "", "", "", "", "", "", "", "", "", "","" };
					modelThuChi.addRow(strs1);
				}
