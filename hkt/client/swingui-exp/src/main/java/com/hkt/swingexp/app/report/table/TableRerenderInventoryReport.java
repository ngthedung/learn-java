...
public class TableRerenderInventoryReport extends AbstractCellEditor implements TableCellRenderer, TableCellEditor {
  ...
  public TableRerenderInventoryReport(TableModelReportInventory model1, Date startDate, Date endDate, String valueMoney, String warehouse, boolean showAll) {   
				...
				try {
					...
					if (index <= inventoryEntity.getIndex()) {
						...
						if (inventoryEntity.getListChild() == null) {
							...
							reportRestaurantEntities.addAll(getProducts(inventoryEntity.getParent(), warehouse, inventoryEntity.getIndex())); // Add All Product
							if (!showAll) {
				...
				try {
					...
					if (index <= inventoryEntity.getIndex()) {
						...
						if (inventoryEntity.getListChild() == null) {
							...
							inventoryReportEntities.addAll(getProductTags(inventoryEntity.getParent(), inventoryEntity.getIndex())); // Add All ProductTag
							inventoryReportEntities.addAll(getProducts(inventoryEntity.getParent(), warehouse, inventoryEntity.getIndex())); // Add All Product
							if (!showAll) {
    ...
    if (warehouse == null) {
    	//Lấy giá trị và số lượng đầu kỳ
  		...
  		.field("COALESCE(SUM(i.increaseQuantity-i.reductionQuantity), 0) AS `SLDauKy`", "value2")
  		.field("COALESCE(SUM(i.increaseValue-i.reductionValue), 0) AS `GTDauKy`", "value3")
  		...
  		.groupBy("`productTag`");
  		if(tag != null){
  			rQuery1.cond("SUBSTRING_INDEX(a.value, ':', " + (index) + ") = '" + tag + "'")
  		...
  		.field("COALESCE(SUM(i.reductionQuantity), 0) AS `SLTrongKyGiam`", "value4")
  		.field("COALESCE(SUM(i.reductionValue), 0) AS `GTTrongKyGiam`", "value5")
  		.cond("i.startDate >= '"+dateFormat.format(calendarStart.getTime())+"'")
  		...
  		.groupBy("`productTag`");
  		if(tag != null){
  			rQuery2.cond("SUBSTRING_INDEX(a.value, ':', " + (index) + ") = '" + tag + "'")
  			.cond("(LENGTH(SUBSTRING_INDEX(a.value, ':', " + (index + 1) + ")) - LENGTH(REPLACE(SUBSTRING_INDEX(a.value, ':', " + (index + 1) + "), ':', '')))  >= " + (index));
  		}
  		...
  		.field("SUBSTRING_INDEX(a.value, ':', " + (index + 1) + ") AS `productTag`", "value0")
  		.field("(SELECT w.label FROM warehouse_productTag w WHERE w.code = SUBSTRING_INDEX(`productTag`, ':', -1))", "value1")
  		.field("COALESCE(SUM(i.increaseQuantity-i.reductionQuantity), 0) AS `SLConLai`", "value2")
  		.field("COALESCE(SUM(i.increaseValue-i.reductionValue), 0) AS `GTConLai`", "value3")
  		.cond("i.startDate >= '"+dateFormat.format(calendarEnd.getTime())+"'")
  		...
  		.groupBy("`productTag`");
  		if(tag != null){
  			rQuery3.cond("SUBSTRING_INDEX(a.value, ':', " + (index) + ") = '" + tag + "'")
  			.cond("(LENGTH(SUBSTRING_INDEX(a.value, ':', " + (index + 1) + ")) - LENGTH(REPLACE(SUBSTRING_INDEX(a.value, ':', " + (index + 1) + "), ':', '')))  >= " + (index));
  		}
    ...
    if(objects1 != null && objects1.size() > 0){
    	for(int i = 0; i < objects1.size(); i++){
    		InventoryReportEntity obj = new InventoryReportEntity(objects1.get(i).getCol1_DanhMuc(), objects1.get(i).getCol2_SLDauKy(), objects1.get(i).getCol3_GTDauKy(), "0", "0", "0", "0", "0", "0");
    ...
    if(objects2 != null && objects2.size() > 0){
    	for(int i = 0; i < objects2.size(); i++){
    		if(row_hash.containsKey(objects2.get(i).getParent())){
    			...
    			InventoryReportEntity obj = new InventoryReportEntity(objects2.get(i).getCol1_DanhMuc(), "0", "0", objects2.get(i).getCol2_SLDauKy(), objects2.get(i).getCol3_GTDauKy(), objects2.get(i).getCol4_SLTang(), objects2.get(i).getCol5_GTTang(), "0", "0");
    ...
    if(objects3 != null && objects3.size() > 0){
    	for(int i = 0; i < objects3.size(); i++){
    		if(row_hash.containsKey(objects3.get(i).getParent())){
    			...
    			InventoryReportEntity obj = new InventoryReportEntity(objects3.get(i).getCol1_DanhMuc(), "0", "0", "0", "0", "0", "0", objects3.get(i).getCol2_SLDauKy(), objects3.get(i).getCol3_GTDauKy());
		...
		while (in.hasNext()) {
  		...
  		InventoryReportEntity row = new InventoryReportEntity(i.getCol1_DanhMuc(), Double.toString(SLDauKy), Double.toString(GTDauKy), Double.toString(SLTang), Double.toString(GTTang), Double.toString(SLGiam), Double.toString(GTGiam), Double.toString(SLConLai), Double.toString(GTConLai));
  ...
  private List<InventoryReportEntity> getProducts(String tag, String warehouse, int index) throws Exception {
  	if(startDate == null)
      startDate = new Date();
    if(endDate == null)
      endDate = new Date();
    ...
    if(objects1 != null && objects1.size() > 0){
    	for(int i = 0; i < objects1.size(); i++){
    		row_hash.put(objects1.get(i).getParent(), new InventoryReportEntity(objects1.get(i).getCol1_DanhMuc(), objects1.get(i).getCol2_SLDauKy(), objects1.get(i).getCol3_GTDauKy(), "0", "0", "0", "0", "0", "0"));
    	}
    }
    if(objects2 != null && objects2.size() > 0){
    	for(int i = 0; i < objects2.size(); i++){
    		if(row_hash.containsKey(objects2.get(i).getParent())){
    			...
    			row_hash.put(objects2.get(i).getParent(), new InventoryReportEntity(objects2.get(i).getCol1_DanhMuc(), "0", "0", objects2.get(i).getCol2_SLDauKy(), objects2.get(i).getCol3_GTDauKy(), objects2.get(i).getCol4_SLTang(), objects2.get(i).getCol5_GTTang(), "0", "0"));
    		}
    	}
    }
    if(objects3 != null && objects3.size() > 0){
	...
	private List<InventoryReportEntity> runQuery(SQLSelectQuery rQuery) throws Exception {
		...
		for (int i = 0; i < records.size(); i++) {
			...
			if(rQuery.getFields().size() == 6)
				object = new InventoryReportEntity(values[1].toString(), values[2].toString(), values[3].toString(), values[4].toString(), values[5].toString(), "", "", "", "");
			else object = new InventoryReportEntity(values[1].toString(), values[2].toString(), values[3].toString(), "", "", "", "", "", "");
			...
			try {
				...
				if (((InventoryReportEntity) value).getType() == InventoryReportEntity.PRODUCT_TAG) {
					...
					if (((InventoryReportEntity) value).getType() == InventoryReportEntity.PRODUCT) {
						valueQuantity = Double.parseDouble(((InventoryReportEntity) value).getCol10_SLHienTai());
						if (valueQuantity < 0) {
