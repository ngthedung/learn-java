...
public class TableRerenderRevenue extends AbstractCellEditor implements TableCellEditor, TableCellRenderer {
	...
	public TableRerenderRevenue(ModelTableReportRevenue model, String typeReport, HashMap<String, String> hash,
	...
	private void mouseClick_CellTable() {
		...
		if (cell.getRow() < model.getRowCount() - 1)
			indexRowAfter = ((ReportRevenue) model.getValueAt(cell.getRow() + 1, 0)).getIndex();
		if (((ReportRevenue) parent).getType().equals(Type.ROOT)) {
			try {
				ReportRevenue rowCurrent = (ReportRevenue) parent;
				if (indexRowAfter <= rowCurrent.getIndex()) {
					int i = cell.getRow() + 1;
					if (rowCurrent.getListChild() == null) {
						List<ReportRevenue> list = new ArrayList<ReportRevenue>();
						list = getParentByPath(null);
						System.out.println("DATEEEEEE STARTTTT " + dateStart);
						System.out.println("DATEEEEEE ENDD " + dateEnd);
						// if(!conds.containsKey("department"))
						// list.addAll(getChildByPath(null));
		...
		if (((ReportRevenue) parent).getType().equals(Type.PARENT)) {
			try {
				ReportRevenue rowCurrent = (ReportRevenue) parent;
				if (indexRowAfter <= rowCurrent.getIndex()) {
					int i = cell.getRow() + 1;
					if (rowCurrent.getListChild() == null) {
						...
						if (conds.containsKey("department")) {
							...
							if (conds.containsKey("location1"))
								list = getChildByPath(rowCurrent);
							else {
		...
		if (((ReportRevenue) parent).getType().equals(Type.CHILD) && conds.containsKey("location1")) {
			try {
				ReportRevenue rowCurrent = (ReportRevenue) parent;
				if (indexRowAfter <= rowCurrent.getIndex()) {
					int i = cell.getRow() + 1;
	 ...
	 */
	private List<ReportRevenue> getParentByPath(ReportRevenue reportRevenue) throws Exception {
		List<ReportRevenue> nodes = new ArrayList<ReportRevenue>();
		String parent = null;
		int index = 1;
		if (reportRevenue != null) {
			parent = reportRevenue.getParent();
			index = index + reportRevenue.getIndex();
		}
		...
		if (typeReport.equals("Doanh thu"))
			rQuery = query_Parent_DoanhThu_ChiPhi(reportRevenue, parent, index, ActivityType.Receipt);
		if (typeReport.equals("Chi phí"))
			rQuery = query_Parent_DoanhThu_ChiPhi(reportRevenue, parent, index, ActivityType.Payment);
		if (typeReport.equals("Thu - Chi - Lãi"))
			rQuery = query_Parent_ThuChiLai(parent, index, false);
		if (typeReport.equals("Thu - Chi - Công nợ")) {
			rQueryR = query_Parent_DoanhThu_ChiPhi(reportRevenue, parent, index, ActivityType.Receipt);
			rQueryP = query_Parent_DoanhThu_ChiPhi(reportRevenue, parent, index, ActivityType.Payment);
		}
		...
		if (!typeReport.equals("Thu - Chi - Công nợ")) {
			...
			for (int i = 0; i < records.size(); i++) {
				...
				if (typeReport.equals("Thu - Chi - Công nợ")) {
					...
					row = new ReportRevenue(values[0].toString(), Double.toString(sumFinalCharge),
					...
					if (values[0] != null) {
						if (conds.containsKey("store")) {
							...
							row = new ReportRevenue(name, Double.toString(sumFinalCharge), Double.toString(sumTotalTransaction),
							    Double.toString(ratio), index);
						} else {
							row = new ReportRevenue(values[0].toString(), Double.toString(sumFinalCharge),
			...
			List<ReportRevenue> lstReceipt = null;
			List<ReportRevenue> lstPyament = null;

			lstReceipt = runQuery(rQueryR);
			lstPyament = runQuery(rQueryP);

			HashMap<String, ReportRevenue> row_hash = new HashMap<String, ReportRevenue>();
			if (lstReceipt != null && lstReceipt.size() > 0) {
				for (int i = 0; i < lstReceipt.size(); i++) {
					ReportRevenue obj = new ReportRevenue(lstReceipt.get(i).getNode(), lstReceipt.get(i).getColumn1(), lstReceipt
			...
			if (lstPyament != null && lstPyament.size() > 0) {
				for (int i = 0; i < lstPyament.size(); i++) {
					if (!row_hash.containsKey(lstPyament.get(i).getParent())) {
						ReportRevenue obj = new ReportRevenue(lstPyament.get(i).getNode(), "0", "0",
						...
						ReportRevenue re = row_hash.get(lstPyament.get(i).getParent());
			...
			Iterator<ReportRevenue> rr = row_hash.values().iterator();
			while (rr.hasNext()) {
				ReportRevenue r = rr.next();
				...
				if (conds.containsKey("store")) {
					...
					ReportRevenue row = new ReportRevenue(name, Double.toString(tongThu), Double.toString(tongChi),
					...
					ReportRevenue row = new ReportRevenue(r.getNode(), Double.toString(tongThu), Double.toString(tongChi),
					    Double.toString(thucThu), Double.toString(thucChi), Double.toString(phaiThu), Double.toString(phaiChi),
					    Double.toString(lai), index);
					row.setParent(r.getParent().toString());
					row.setType(Type.PARENT);
	...
	private SQLSelectQuery query_Parent_DoanhThu_ChiPhi(ReportRevenue reportRevenue, String parent, int index,
	 ...
	 */
	private List<ReportRevenue> getChildByPath(ReportRevenue reportRevenue) throws Exception {
		List<ReportRevenue> nodes = new ArrayList<ReportRevenue>();
		String parent = null;
		int index = 1;
		if (reportRevenue != null) {
		...
		if (typeReport.equals("Doanh thu") || typeReport.equals("Chi phí") || typeReport.equals("Thu - Chi - Lãi")) {
			SQLSelectQuery rQuery = new SQLSelectQuery();
			if (typeReport.equals("Doanh thu"))
				rQuery = query_Child_DoanhThu_ChiPhi(reportRevenue, parent, index, ActivityType.Receipt);
			if (typeReport.equals("Chi phí"))
				rQuery = query_Child_DoanhThu_ChiPhi(reportRevenue, parent, index, ActivityType.Payment);
			if (typeReport.equals("Thu - Chi - Lãi"))
				rQuery = query_Child_ThuChiLai(reportRevenue, parent, index, false);
			...
			for (int i = 0; i < records.size(); i++) {
				...
				ReportRevenue n = new ReportRevenue(values[0].toString(), MyDouble.valueOf(sumFinalCharge).toString(), MyDouble
		 ...
		 ******************************/
		if (typeReport.equals("Thu - Chi - Công nợ")) {
			SQLSelectQuery rQueryR = query_Child_DoanhThu_ChiPhi(reportRevenue, parent, index, ActivityType.Receipt);
			SQLSelectQuery rQueryP = query_Child_DoanhThu_ChiPhi(reportRevenue, parent, index, ActivityType.Payment);

			List<ReportRevenue> lstReceipt = null;
			List<ReportRevenue> lstPayment = null;
			...
			if (lstPayment != null && lstPayment.size() > 0) {
				for (int i = 0; i < lstPayment.size(); i++) {
					if (!row_hash.containsKey(lstPayment.get(i).getNode())) {
						ReportRevenue obj = new ReportRevenue(lstPayment.get(i).getNode(), "0", "0",
						...
						ReportRevenue re = row_hash.get(lstPayment.get(i).getNode());
	...
	private SQLSelectQuery query_Child_DoanhThu_ChiPhi(ReportRevenue reportRevenue, String parent, int index,
	    ActivityType activityType) {
		SQLSelectQuery rQuery = new SQLSelectQuery();
		String sql = "";
		if (isCheck == true) {
	...
	private SQLSelectQuery query_Child_ThuChiLai(ReportRevenue reportRevenue, String parent, int index,
	...
	//TODO
	private List<ReportRevenue> getInvoice(ReportRevenue reportRevenue) throws Exception {
		List<ReportRevenue> nodes = new ArrayList<ReportRevenue>();
		...
		if (typeReport.equals("Doanh thu")) {
			...
			for (int i = 0; i < records.size(); i++) {
				...
				ReportRevenue n = new ReportRevenue("Hóa đơn " + values[0].toString(), Double.toString(sumFinalCharge),
	...
	private List<ReportRevenue> runQuery(SQLSelectQuery rQuery) throws Exception {
		List<ReportRevenue> listObjects = new ArrayList<ReportRevenue>();
		System.out.println(rQuery.createSQLQuery());
		ReportTable[] reportTable = accountingServiceClient.reportQuery(new SQLSelectQuery[] { rQuery });
		String[] field = new String[] { "node", "final", "total", "parent" };
		...
		for (int i = 0; i < records.size(); i++) {
			...
			if (values[3] != null) {
				ReportRevenue object = new ReportRevenue(values[0].toString(), values[1].toString(), values[2].toString(),
				    values[3].toString(), 0);
	...
	public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus,
		...
		if (column == 0) {
			...
			String result = " ";
			((ReportRevenue) value).setLabelIcon(labelIcon);
			try {
				int index = ((ReportRevenue) value).getIndex();
				if (((ReportRevenue) value).isWiden())
				...
				if (((ReportRevenue) value).getType().equals(Type.CHILD)) {
					if (conds.containsKey("product1"))
				...
				if (((ReportRevenue) value).getType().equals(Type.INVOICE))
	...
	public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int row, int column) {
		...
		if (column == 0) {
			JLabel labelIcon = ((ReportRevenue) value).getLabelIcon();
			try {
				...
				new SwingWorker<Void, Void>() {
					...
					protected void done() {
						...
						List<ReportRevenue> list = ((ReportRevenue) c).getListChild();
						...
						if (typeReport.equals("Doanh thu") || typeReport.equals("Chi phí")) {
							...
							if (type.equals("Tổng thu") || type.equals("Tổng chi")) {
								for (ReportRevenue r : list) {
									Double d = (new MyDouble(r.getColumn1()).doubleValue()
									    / new MyDouble(((ReportRevenue) c).getColumn1()).doubleValue() + new MyDouble(r.getColumn1())
									    .doubleValue() % new MyDouble(((ReportRevenue) c).getColumn1()).doubleValue()) * 100;
								...
								if (type.equals("Thực thu") || type.equals("Thực chi")) {
									for (ReportRevenue r : list) {
										Double d = (new MyDouble(r.getColumn2()).doubleValue()
										    / new MyDouble(((ReportRevenue) c).getColumn2()).doubleValue() + new MyDouble(r.getColumn2())
										    .doubleValue() % new MyDouble(((ReportRevenue) c).getColumn2()).doubleValue()) * 100;
										dataset.setValue(r.getNode(), d);
									}
								} else {
									if (type.equals("Tỉ lệ(%)")) {
										for (ReportRevenue r : list) {
											Double d = (new MyDouble(r.getColumn3()).doubleValue()
											    / new MyDouble(((ReportRevenue) c).getColumn3()).doubleValue() + new MyDouble(r.getColumn3())
											    .doubleValue() % new MyDouble(((ReportRevenue) c).getColumn3()).doubleValue()) * 100;
											dataset.setValue(r.getNode(), d);
										}
									}
								}
										...
										if (type.equals("Thực chi")) {
											for (ReportRevenue r : list) {
												Double d = (new MyDouble(r.getColumn4()).doubleValue()
												    / new MyDouble(((ReportRevenue) c).getColumn4()).doubleValue() + new MyDouble(
												    r.getColumn4()).doubleValue()
												    % new MyDouble(((ReportRevenue) c).getColumn4()).doubleValue()) * 100;
												dataset.setValue(r.getNode(), d);
											}
										} else {
											if (type.equals("Phải thu")) {
												for (ReportRevenue r : list) {
													Double d = (new MyDouble(r.getColumn5()).doubleValue()
													    / new MyDouble(((ReportRevenue) c).getColumn5()).doubleValue() + new MyDouble(
													    r.getColumn5()).doubleValue()
													    % new MyDouble(((ReportRevenue) c).getColumn5()).doubleValue()) * 100;
													dataset.setValue(r.getNode(), d);
												}
											} else {
												if (type.equals("Phải chi")) {
													for (ReportRevenue r : list) {
														Double d = (new MyDouble(r.getColumn6()).doubleValue()
														    / new MyDouble(((ReportRevenue) c).getColumn6()).doubleValue() + new MyDouble(
														    r.getColumn6()).doubleValue()
														    % new MyDouble(((ReportRevenue) c).getColumn6()).doubleValue()) * 100;
														dataset.setValue(r.getNode(), d);
													}
												} else {
													if (type.equals("Lãi(lỗ)")) {
														for (ReportRevenue r : list) {
															Double d = (new MyDouble(r.getColumn7()).doubleValue()
															    / new MyDouble(((ReportRevenue) c).getColumn7()).doubleValue() + new MyDouble(
															    r.getColumn7()).doubleValue()
															    % new MyDouble(((ReportRevenue) c).getColumn7()).doubleValue()) * 100;
															dataset.setValue(r.getNode(), d);
														}
													}
												}
