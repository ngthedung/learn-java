...
function createReceipt(frameui) {
	frameui.showDialogTest("Chương trình bắt đầu chạy thử giao diện [Phiếu Chi Tiền]");
	frameui.delay();
	var buttonMenu = frameui.button("menuRightThuChiMuaHang");
	buttonMenu.mouseClick();
	var button = frameui.button("PhieuChiTien");
	button.mouseClick()

	var dlMain = frameui.dialog("dlPhieuChiTien");
	var pnChild = dlMain.panel("PhieuChiTien");
...
function createListPayment(frameui) {
	...
	frameui.showDialogTest("Chạy thử chức năng [Xem] phiếu chi tiền khi lưu thành công");
	frameui.delay();
	...
	function createReceipt1(frameui) {
		frameui.showDialogTest("[Thêm mới] 1 Phiếu chi tiền");
		frameui.delay();
		var buttonMenu = frameui.button("menuRightThuChiMuaHang");
		buttonMenu.mouseClick();
		var button = frameui.button("PhieuChiTien");
	...
	function createListPayment1(frameui) {
		...
		frameui.showDialogTest("Chương trình TEST [Phiếu Chi Tiền] kết thúc!");
		frameui.delay();
		var dlMain = frameui.dialog("dlinvoice");
		dlMain.buttonClick("Thoát");
	}
...
var robot = new Robot();
robot.add("TEST PhieuChiTien ", "", createReceipt);
robot.add("TEST DSThuChi ", "", createListPayment);
robot.add("TEST PhieuChiTien ", "", createReceipt1);
