...
public class TableRerenderQuantitative extends AbstractCellEditor implements TableCellRenderer, TableCellEditor {
	...
	public TableRerenderQuantitative(TableModelReportInventory model1, final Date startDate, final Date endDate,
	    String valueMoney, String warehouse, boolean showAll, HashMap<String, InventoryReportEntity> rowH) {
					...
					if (index <= inventoryEntity.getIndex()) {
						...
						if (inventoryEntity.getListChild() == null) {
							...
							reportRestaurantEntities.addAll(getProductTags(tag, index)); // Add
							                                                             // All
							                                                             // ProductTag
							// reportRestaurantEntities.addAll(getProducts(inventoryEntity.getParent(),
							// inventoryEntity.getIndex())); // Add
							// All
							// Product
							// if (!showAll) {
							for (int ep = 0; ep < reportRestaurantEntities.size();) {
								if (!reportRestaurantEntities.get(ep).isNotRowZero()
								    || ProductModelManager.getInstance()
								        .getProductTagByCode(reportRestaurantEntities.get(ep).getParent()).getParentTag() != null) {
				...
				try {
					...
					if (index <= inventoryEntity.getIndex()) {
						...
						if (inventoryEntity.getListChild() == null) {
							...
							// inventoryReportEntities.addAll(getProducts(inventoryEntity.getParent(),
							// inventoryEntity.getIndex()));
							if (!showAll) {
