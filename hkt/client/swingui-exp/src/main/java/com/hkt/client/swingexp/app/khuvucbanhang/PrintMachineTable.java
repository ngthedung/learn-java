...
import com.hkt.client.swingexp.app.khuvucbanhang.list.PanelListTablePrint;
import com.hkt.client.swingexp.app.khuvucbanhang.list.TableListTablePrintMachine;
...
public class PrintMachineTable extends JPanel implements IDialogResto {

	private PanelTop panelTop;
	private PanelCenter panelCenter;
	private PanelListTablePrint panelListTable;
	private IDialogResto iDialogResto;
	private TableListTablePrintMachine tableTable;
	...
	/**
	 * Create the panel PrintMachineTable
	 */
	public PrintMachineTable() {
		...
		tableTable.setTable(printMachine.getTables());
		cbPaperSize.setSelectedItem((String) printMachine.getTemplate());
		listTable = printMachine.getTables();
	}
	...
	protected class PanelTop extends JPanel {
		...
		private String[] paperSize = { PrintMachine.TEMPLATE_A4,
				PrintMachine.TEMPLATE_A5, PrintMachine.TEMPLATE_A6,
		...
		public PanelTop() {
			...
			cbPrinter.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					printMachine = (PrintMachine) cbPrinter.getSelectedItem();
					tableTable.setTable(printMachine.getTables());
					cbPaperSize.setSelectedItem((String) printMachine
							.getTemplate());
					listTable = printMachine.getTables();
				}
			});
		...
		public void init() {
			...
			try {
				List<PrintMachine> listPrintMachine = PrintMachineModelManager
						.getInstance().getPrintMachines();
				DefaultComboBoxModel<PrintMachine> modelPrintMachine = new DefaultComboBoxModel(
						listPrintMachine.toArray());
				cbPrinter.setModel(modelPrintMachine);
			} catch (Exception e) {
				e.printStackTrace();
			}
			DefaultComboBoxModel<String> modelPaperSize = new DefaultComboBoxModel<String>(
					paperSize);
	...
	protected class PanelCenter extends JPanel {
		...
		public void init() {
			...
			btnEdit.setIcon(new ImageIcon(HomeScreen.class
					.getResource("icon/modify1.png")));
			btnEdit.addMouseListener(new MouseEventClickButtonDialog(
					"modify1.png", "modify2.png", "modify3.png"));
			btnEdit.addActionListener(new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent e) {
					try {
//						if (UIConfigModelManager.getInstance().getPermission(
//								permisson) == Capability.READ) {
//							JOptionPane.showMessageDialog(null,
//									"Bạn chưa được cấp quyền này !");
//							return;
//						} else {
							panelListTable = new PanelListTablePrint();
							panelListTable.setName("panel");
							if (listTable == null) {
								listTable = new ArrayList<Table>();
							}
							panelListTable.getData(listTable);
							panelListTable.setParentForm(iDialogResto);
							if (Toolkit.getDefaultToolkit().getScreenSize().width <= 1024) {
								DialogResto dialog = new DialogResto(
										panelListTable, false, 880, 450);
...
//						}
			...
			tableTable = new TableListTablePrintMachine(new ArrayList<Table>());
	...
	public void loadTable(List<Table> listTable) {
		if (listTable != null)
			this.listTable = listTable;
		else
			listTable = new ArrayList<Table>();
		tableTable.setTable(listTable);
	}

	private void setData() {
		printMachine.setTemplate(template);
		printMachine.setTables(listTable);
