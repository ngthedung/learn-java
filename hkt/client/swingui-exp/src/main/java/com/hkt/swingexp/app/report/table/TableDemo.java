...
public class TableDemo extends JTable {
	...
	public TableDemo(int typeReport, int setView, HashMap<String, String> hash, String viewUnitMoney) {
		this.conds = hash;
		...
		try {
			if (typeReport == 2) {
				if (conds.containsKey("product1"))
					model = new ModelTableDemo(4, getNodeParent());
				else
					model = new ModelTableDemo(typeReport, getNodeParent());
			} else
				model = new ModelTableDemo(typeReport, getNodeParent());

			this.setModel(model);
			for (int i = 0; i < this.getColumnCount(); i++) {
				this.getColumnModel().getColumn(i).setCellRenderer(new TableRerenderDemo(model, typeReport, setView, hash, viewUnitMoney));
				this.getColumnModel().getColumn(i).setCellEditor(new TableRerenderDemo(model, typeReport, setView, hash, viewUnitMoney));
			}
	...
	private List<DemoEntity> getNodeParent() throws Exception {
	...
	private DemoEntity getRowFromQuery(String[] row){	
		double sumFinal = 0;
		double sumTotal = 0;
		double sumFinal0 = 0;
		double sumTotal0 = 0;
		...
		if(row.length <= 2){
			...
			entity = new DemoEntity(nameNode, new MyDouble(sumFinal).toString(), new MyDouble(sumTotal).toString(), new MyDouble(ratio).toString(), 0);
		} else {
			ratio = sumTotal - sumTotal0;
			entity = new DemoEntity(nameNode, new MyDouble(sumFinal).toString(), new MyDouble(sumFinal0).toString(), new MyDouble(sumTotal).toString(), new MyDouble(sumTotal0).toString(), new MyDouble(sumPhaiThu).toString(), new MyDouble(sumPhaiChi).toString(), new MyDouble(ratio).toString(), 0);
		}
