...
import com.hkt.swingexp.app.report.entity.DemoEntity.Type;
import com.hkt.swingexp.app.report.entity.PanelTableCell;
import com.hkt.swingexp.app.report.modeltable.ModelTableDemo;

public class TableRerenderDemo extends AbstractCellEditor implements TableCellRenderer, TableCellEditor {
	private PanelTableCell									cell;
	private ModelTableDemo								model;
  ...
  public TableRerenderDemo(ModelTableDemo model, int typeReport, int setView, HashMap<String, String> hash, String viewMoney) {
	...
	public void mouseClick_CellTable() {
		try {
			...
			if (cell.getRow() < model.getRowCount() - 1) {
				index = ((DemoEntity) model.getValueAt(cell.getRow() + 1, 0)).getIndex();
			}
			try {
				DemoEntity rowCurrent = (DemoEntity) parent;
				if (index <= rowCurrent.getIndex()) {
					...
					if (rowCurrent.getListChild() == null) {
						...
						List<DemoEntity> listChilds = new ArrayList<DemoEntity>();
						banks = AccountingModelManager.getInstance().getBankByCode(rowCurrent.getNode());
//						System.out.println("BANKKKKKKK  " + banks.getParentCode());
						if (rowCurrent.getNode().equals("Nhà hàng Grill 66")){
							lsBank = AccountingModelManager.getInstance().getBanks(1);
								listChilds.add(new DemoEntity(lsBank.get(0).getBankCode(), lsBank.get(0).getName(), returnTotal("ts"), "0", indexs));
								listChilds.add(new DemoEntity(lsBank.get(1).getBankCode(), lsBank.get(1).getName(), returnTotal("nv"), "0", indexs));
						} else {
//							System.out.println(rowCurrent.getNode() + "   " + rowCurrent.getColumn1());
							lsBankName = AccountingModelManager.getInstance().getByBankName(rowCurrent.getColumn1());
							for (int k=0; k<lsBankName.size(); k++){
//								System.out.println(" NAMEEEEEEE  " + lsBankName.get(k).getName());
								if (lsBankName.get(k).getBankCode().equals(rowCurrent.getNode())){
//									System.out.println(" OKKKKKKKKKKKKKKK  " + lsBankName.get(k).getName() + "    " + lsBankName.get(k).getParentCode());
									lsBank = AccountingModelManager.getInstance().getBanks(returnIndex(lsBankName.get(k).getCode()));
									for (int j=0; j<lsBank.size(); j++){
										listChilds.add(new DemoEntity(lsBank.get(j).getBankCode(), lsBank.get(j).getName(), returnTotal(lsBankName.get(k).getCode() + lsBank.get(j).getBankCode()), "0", indexs));
										System.out.println("BANKCODEEEEEE  " + lsBankName.get(k).getCode() + lsBank.get(j).getBankCode());
									}
								}
							}
						}
	...
	private String returnTotal(String code){
  ...
  private SQLSelectQuery SQLQueryString_GetMonth(DemoEntity objectParent, ActivityType activityType, String parameter){
  	SQLSelectQuery rQuery = new SQLSelectQuery();
  	/**
  	 ****************************************
  	 **************************************** 
  ...
  private SQLSelectQuery SQLQueryString_GetDay(DemoEntity objectParent, ActivityType activityType, String parameter){
  	SQLSelectQuery rQuery = new SQLSelectQuery();
  	/**
  	 ****************************************
  	 **************************************** 
  ...
  private SQLSelectQuery SQLQueryString_GetHour(DemoEntity objectParent, ActivityType activityType, String parameter){
	  	SQLSelectQuery rQuery = new SQLSelectQuery();
  ...
  private SQLSelectQuery SQLQueryString_GetShift(DemoEntity objectParent, ActivityType activityType, String parameter){
	  	SQLSelectQuery rQuery = new SQLSelectQuery();
	  	/**
	  	 ****************************************
	  	 **************************************** 
  ...
  private SQLSelectQuery SQLQueryString_GetInvoice(DemoEntity objectParent, ActivityType activityType, String parameter){
	  	SQLSelectQuery rQuery = new SQLSelectQuery();
	  	/**
	  	 ****************************************
	  	 **************************************** 
  ...
  private List<DemoEntity> getRows(DemoEntity entityForTime, Type type) throws Exception {
	  List<DemoEntity> nodes = new ArrayList<DemoEntity>();
		int index = 1;
		...
		for (int i = 0; i < records.size(); i++) {
			...
			if(typeReport == 3){ //Nếu là kiểu thu chi công nợ
				...
				row = new DemoEntity(values[0].toString(), new MyDouble(sumFinalCharge).toString(), new MyDouble(sumTotalTransaction).toString(), new MyDouble(ratio).toString(), index);
				if(setView == 2 || setView == 3)
				{
				if(type.equals(Type.ROOT)){
					row.setType(Type.NODE_YEAR);
				...
				if(setView ==2 || setView ==3){
					if(type.equals(Type.NODE_SHIFT))
					{
						...
						row = new DemoEntity(str+ "h", new MyDouble(sumFinalCharge).toString(), new MyDouble(sumTotalTransaction).toString(), new MyDouble(ratio).toString(), index);
						row.setType(Type.NODE_HOUR);
  ...
  public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
		...
		if(column == 0){
			...
			String result = " ";
			((DemoEntity) value).setLabelIcon(labelIcon);
			int index = ((DemoEntity) value).getIndex();
			try {
	      cell.setObject(value);
				if(((DemoEntity) value).isWiden())
  ...
  public Component getTableCellEditorComponent(JTable table, Object value, boolean isSelected, int row, int column) {
    ...
    final Object c = value;
		if (column == 0) {
			JLabel labelIcon = ((DemoEntity) value).getLabelIcon();
			 try {
				 processing.setVisible(true);
					new SwingWorker<Void, Void>() {
						...
						protected void done() {
							...
							List<DemoEntity> list = ((DemoEntity) c).getListChild();
							...
							if (typeReport == 0 || typeReport == 1){
								...
								if(type.equals("Tổng thu") || type.equals("Tổng chi")){
									for(DemoEntity r : list){
										Double d = (new MyDouble(r.getColumn1()).doubleValue()/new MyDouble(((DemoEntity) c).getColumn1()).doubleValue() + new MyDouble(r.getColumn1()).doubleValue()%new MyDouble(((DemoEntity) c).getColumn1()).doubleValue())*100;
										dataset.setValue(r.getNode(), d);
									}
								} else {
									if(type.equals("Thực thu") || type.equals("Thực chi")){
										for(DemoEntity r : list){
											Double d = (new MyDouble(r.getColumn2()).doubleValue()/new MyDouble(((DemoEntity) c).getColumn2()).doubleValue() + new MyDouble(r.getColumn2()).doubleValue()%new MyDouble(((DemoEntity) c).getColumn2()).doubleValue())*100;
											dataset.setValue(r.getNode(), d);
										}
									} else {
										if(type.equals("Tỉ lệ(%)")){
											for(DemoEntity r : list){
												Double d = (new MyDouble(r.getColumn3()).doubleValue()/new MyDouble(((DemoEntity) c).getColumn3()).doubleValue() + new MyDouble(r.getColumn3()).doubleValue()%new MyDouble(((DemoEntity) c).getColumn3()).doubleValue())*100;
												dataset.setValue(r.getNode(), d);
											}
										}
									}
								...
								if(type.equals("Tổng thu")){
									...
									if(type.equals("Tổng chi")){
										...
										if(type.equals("Thực thu")){
											...
											if(type.equals("Thực chi")){
												for(DemoEntity r : list){
													Double d = (new MyDouble(r.getColumn4()).doubleValue()/new MyDouble(((DemoEntity) c).getColumn4()).doubleValue() + new MyDouble(r.getColumn4()).doubleValue()%new MyDouble(((DemoEntity) c).getColumn4()).doubleValue())*100;
													dataset.setValue(r.getNode(), d);
												}
											} else {
												if(type.equals("Phải thu")){
													for(DemoEntity r : list){
														Double d = (new MyDouble(r.getColumn5()).doubleValue()/new MyDouble(((DemoEntity) c).getColumn5()).doubleValue() + new MyDouble(r.getColumn5()).doubleValue()%new MyDouble(((DemoEntity) c).getColumn5()).doubleValue())*100;
														dataset.setValue(r.getNode(), d);
													}
												} else {
													if(type.equals("Phải chi")){
														for(DemoEntity r : list){
															Double d = (new MyDouble(r.getColumn6()).doubleValue()/new MyDouble(((DemoEntity) c).getColumn6()).doubleValue() + new MyDouble(r.getColumn6()).doubleValue()%new MyDouble(((DemoEntity) c).getColumn6()).doubleValue())*100;
															dataset.setValue(r.getNode(), d);
														}
													} else {
														if(type.equals("Lãi(lỗ)")){
															for(DemoEntity r : list){
																Double d = (new MyDouble(r.getColumn7()).doubleValue()/new MyDouble(((DemoEntity) c).getColumn7()).doubleValue() + new MyDouble(r.getColumn7()).doubleValue()%new MyDouble(((DemoEntity) c).getColumn7()).doubleValue())*100;
																dataset.setValue(r.getNode(), d);
															}
														}
													}
