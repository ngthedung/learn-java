...
public class TestModelTableReportRevenue extends DefaultTableModel {
	private List<ReportRevenueTest>	listReport	= new ArrayList<ReportRevenueTest>();
	private String[]						COLUMN_NAMES;
	private ReportStatistics		reportStatistics;
	
	public TestModelTableReportRevenue(int columnType, List<ReportRevenueTest> listData) {
	...
	public void addRowChild(List<ReportRevenueTest> list, int index) {
	...
	public Object getValueAt(int row, int column) {
		ReportRevenueTest emp = ((ReportRevenueTest) dataVector.get(row));
		switch (column) {
			case 0:
				try {
					return emp;
	...
	public void removeRowReport(int row) {
		ReportRevenueTest object = (ReportRevenueTest) dataVector.get(row);
		int k = row + 1;
		while (((ReportRevenueTest) dataVector.get(k)).getIndex() > object.getIndex()) {
		...
		for (ReportRevenueTest se : listReport.subList(row + 1, k)) {
			if (se.getType() != ReportRevenueTest.Type.CHILD) {
				se.setWiden(false);
			}
		}
