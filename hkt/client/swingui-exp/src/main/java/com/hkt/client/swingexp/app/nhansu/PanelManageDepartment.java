...
public class PanelManageDepartment extends JPanel implements IDialogResto {
	...
	private void init() {
		...
		if  (Toolkit.getDefaultToolkit().getScreenSize().width <= 1024) {
			scrollListLeft.setPreferredSize(new Dimension(300, 80));
			scrollListRight.setPreferredSize(new Dimension(300, 80));
			} else {
				scrollListLeft.setPreferredSize(new Dimension(400, 80));
				scrollListRight.setPreferredSize(new Dimension(400, 80));
				}
	...
	private void setModelListView() {
		try {
			...
			List<AccountGroup> list = AccountModelManager.getInstance().getAllChildrensByPath(groupDepartment.getPath());
	...
	public void getData(List<AccountGroup> accountGroup) {
		...
		try {
			...
			if (listSelected.size() != listTableCurrent.size()) {
				for (AccountGroup acc : listSelected) {
					if(acc!=null && acc.getId()!=null){
					  for (int i = 0; i < listTableCurrent.size(); ) {
						if (listTableCurrent.get(i).getId().longValue() == acc.getId().longValue()){
	...
	public List<AccountGroup> getListDepartment(){
		return listSelected;
	}
	
	public void setListDepartment(List<AccountGroup> listData){
		if(listData != null){
			for(AccountGroup ag : listData){
				listSelectedAccountGroup.addElement(ag);
			}
			listAccGroupRight.setModel(listSelectedAccountGroup);
		}
	}
