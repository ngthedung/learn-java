...
public abstract class MyDefaultRowSorter<M, I> extends RowSorter<M> {
     ...
     */
    private MyRowFilter<? super M,? super I> filter;
     ...
     *        included
     */
    public void setRowFilter(MyRowFilter<? super M,? super I> filter) {
        this.filter = filter;
        sort();
    }
     ...
     * @return the filter
     */
    public MyRowFilter<? super M,? super I> getRowFilter() {
        return filter;
    }
    ...
    private MyRowFilter.Entry<M,I> getFilterEntry(int modelIndex) {
        if (filterEntry == null) {
    ...
    private boolean include(int row) {
      MyRowFilter<? super M, ? super I> filter = getRowFilter();
        if (filter != null) {
     ...
     * the instance.
     */
    private class FilterEntry extends MyRowFilter.Entry<M,I> {
        /**
        ...
        public String getStringValue(int index) {
          String str = Normalizer.normalize(getModelWrapper().getStringValueAt(modelIndex, index), Normalizer.Form.NFD)
