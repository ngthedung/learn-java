...
import com.hkt.client.swingexp.app.khuvucbanhang.list.PanelListProductPrint;
import com.hkt.client.swingexp.app.khuvucbanhang.list.TableListProducts;
...
public class PrintMachineProducts extends JPanel implements IDialogResto {

	private PanelTop panelTop;
	private PanelCenter panelCenter;
	private PanelListProductPrint panelListProducts;
	private IDialogResto iDialogResto;
	private TableListProducts tableProducts;
	...
	public PrintMachineProducts() {
		...
		tableProducts.setProductTags(printMachine.getProductTags());
		cbPaperSize.setSelectedItem((String) printMachine.getTemplate());
		listProductTag = printMachine.getProductTags();
		addKeyBindings(this);
	}
	...
	protected class PanelCenter extends JPanel {
		...
		public void init() {
			...
			btnEdit.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					try {
						// if (UIConfigModelManager.getInstance().getPermission(permisson)
						// == Capability.READ) {
						// JOptionPane.showMessageDialog(null,
						// "Bạn chưa được cấp quyền này !");
						// return;
						// } else {
						panelListProducts = new PanelListProductPrint();
						panelListProducts.setName("panel");
						if (listProductTag == null) {
							listProductTag = new ArrayList<ProductTag>();
						}
						panelListProducts.getData(listProductTag);
						panelListProducts.setParentForm(iDialogResto);

						if (Toolkit.getDefaultToolkit().getScreenSize().width <= 1024) {
							DialogResto dialog = new DialogResto(panelListProducts, false, 880, 450);
							dialog.setName("dialog");
							dialog.setTitle("Chọn nhóm sản phẩm");
							dialog.setIcon("mayin1.png");
							dialog.setModal(true);
							dialog.setLocationRelativeTo(null);
							dialog.setVisible(true);
						} else {
							DialogResto dialog = new DialogResto(panelListProducts, false, 950, 450);
							dialog.setName("dialog");
							dialog.setTitle("Chọn nhóm sản phẩm");
							dialog.setIcon("mayin1.png");
							dialog.setModal(true);
			...
			tableProducts = new TableListProducts(new ArrayList<ProductTag>());

			scrollPaneTable = new JScrollPane();
			scrollPaneTable.setViewportView(tableProducts);
	...
	public void loadTable(List<ProductTag> listProTag) {
		if (listProTag != null)
			this.listProductTag = listProTag;
		else
			listProductTag = new ArrayList<ProductTag>();
		tableProducts.setProductTags(listProductTag);
	}

	private void setData() {
		printMachine.setTemplate(template);
		printMachine.setProductTags(listProductTag);
...
//		} else {
			// if(printMachine != null && listProductTag.size() != 0){
			setData();
