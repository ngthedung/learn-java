define([
  ...
  'ui/UIBean',
  'util/DateTime',
], function($, _, Backbone, i18n, service, UITable, UIPopup, UICollapsible, UIBean, DateTime) {  
  ...
  var UIAccountDetail = UICollapsible.extend({
    ...
    init: function(UIParent, account, readOnly) {
      this.clear() ;
      ...
      this.add(new UIAccount().initWithDetail(account, readOnly)) ;
      if(account.type == "USER") {
        ...
        this.add(new UIUserBasic().init(account.profiles.basic, readOnly)) ;
        ...
        this.add(new UIOrganizationBasic().init(account.profiles.basic, readOnly)) ;
